...
const mediaArtsSearch = { q: "#dumptrump", count: 100, result_type: "recent" };
...
const likeLatest = () => {
  twit.get("search/tweets", mediaArtsSearch, (error, data) => {
    ...
    if (!error) {
      ...
      let retweetId = data.statuses[0].id_str;
      // ...and then we tell Twitter we want to retweet it!
      twit.post("favorites/create" + retweetId, {}, (error, response) => {
        if (response) {
          console.log(
            "Success! Check your bot, it should have favorited something."
          );
        }
        // If there was an error with our Twitter call, we print it out here.
        if (error) {
...
setInterval(likeLatest, 1000 * 60 * 5)
